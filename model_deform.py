...
class REALS_model(nn.Module):
    def __init__(self, data_shape, tau, device, k=1, fix_first=False):
        """
        ...
        if self.tau == 'affine':
            ...
            self.flow0 = torch.zeros(
            1, self.w, self.h, 2, dtype=torch.float, device=device
            ).requires_grad_(True)
            self.flow = torch.zeros(
            self.t - 1, self.w, self.h, 2, dtype=torch.float, device=device
            ).requires_grad_(True)
            print("No decoupling for euclidean transformation!, only deformable!")
            self.stn = SpatialTransformation(False if self.device == "cpu" else True)
    ...
    def forward(self, x):
        ...
        x_reg = F.grid_sample(x_reshape, grid)  # 60x(512x512)
        
        # x_reg = self.stn(x_reg.permute(0, 2, 3, 1), self.flow).permute(0, 3, 1, 2)
        x_reg0 = self.stn(x_reg[0:1, :, :, :].permute(0, 2, 3, 1), self.flow0).view(1, self.inp)
        x_reg1 = self.stn(x_reg[1:, :, :, :].permute(0, 2, 3, 1), self.flow).view(self.t - 1, self.inp)
        x_reg = torch.cat((x_reg0, x_reg1), dim=0)
        x_reg = x_reg.view(self.t, self.inp)

        if self.fix_first:
            if self.k > 1:
                raise RuntimeError
            else:
                tmp =  x_reg[0:1, :].clone().detach()
                tmp = tmp / torch.norm(tmp, p=2)
                self.ln.weight.data = tmp
