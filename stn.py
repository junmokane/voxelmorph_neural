...
class SpatialTransformation(nn.Module):
    ...
    def meshgrid(self, height, width):
        x_t = torch.matmul(
            torch.ones([height, 1]),
            torch.transpose(
                torch.unsqueeze(torch.linspace(0.0, width - 1.0, width), 1), 1, 0
            ),
        )
        y_t = torch.matmul(
            torch.unsqueeze(torch.linspace(0.0, height - 1.0, height), 1),
            torch.ones([1, width]),
        )
    ...
    def interpolate(self, im, x, y):
        ...
        output = torch.sum(
            torch.squeeze(torch.stack([wa * Ia, wb * Ib, wc * Ic, wd * Id], dim=1)), 1
        )
